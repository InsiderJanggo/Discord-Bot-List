...
module.exports.getUser = (token) => {
    return new Promise(async function(resolve, reject) {
        ...
        const response = await fetch(`https://discordapp.com/api/oauth2/token?grant_type=refresh_token&refresh_token=${token}&scope=identify&redirect_url=${encodeURIComponent(DOMAIN)}/api/callback`, {
            method: 'POST',
            headers: {
                Authorization: `Basics ${creds}`,
            },
        });
